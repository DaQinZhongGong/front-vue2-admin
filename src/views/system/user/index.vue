...
export default {
  ...
  created() {
    this.crud.msg.add = '新增成功，默认密码：5t6FrY3LmDK584jqqyV8RBQXrcW7JPPP'
  },
  mounted: function() {
    const that = this
    ...
    resetPwd(datas) {
      this.$confirm(`你选中了 ${datas.length} 位用户，确认重置用户的密码吗?`, '提示', {
        ...
        crudUser.resetPwd(ids).then(() => {
          this.crud.notify('重置成功, 用户新密码:5t6FrY3LmDK584jqqyV8RBQXrcW7JPPP', CRUD.NOTIFICATION_TYPE.SUCCESS)
        }).catch(() => {})
      }).catch(() => {
      })
    }
